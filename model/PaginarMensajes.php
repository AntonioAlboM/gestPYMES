...
class PaginarMensajes extends EntidadBase {
    ...
    public function consultar($inicio){
        $consulta = $this->db()->prepare("SELECT u.nombre,u.apellidos, u.id ,p.idReceptor,p.idEmisor, sum(p.leido) as leido FROM empleados u left JOIN mensajes p ON u.id = p.idEmisor WHERE u.idEmpresa = 30073 group by u.id order by sum(p.leido) desc, u.apellidos limit $inicio, $this->limite ");
        $consulta->execute();
